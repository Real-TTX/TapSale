...
{
 [BindProperty]public long SaleListId{get;set;}[BindProperty,Range(0,1000000)]public decimal Amount{get;set;}[BindProperty,MaxLength(120)]public string? ShiftName{get;set;}public List<SaleList> Lists{get;private set;}=[];public CashShift? Active{get;private set;}public long ExpectedCents{get;private set;}
 ...
 public async Task<IActionResult> OnPostCloseAsync(long id){var shift=await db.CashShift.SingleOrDefaultAsync(x=>x.Id==id&&x.UserId==current.Id&&x.ClosedDate==null);if(shift is null)return NotFound();if(!ModelState.IsValid){await OnGetAsync(shift.SaleListId);return Page();}shift.Name=string.IsNullOrWhiteSpace(ShiftName)?null:ShiftName.Trim();shift.CountedClosingCents=decimal.ToInt64(Amount*100);shift.ClosedDate=DateTimeOffset.UtcNow;shift.UpdateUserId=current.Id;await db.SaveChangesAsync();return RedirectToPage(new{saleListId=shift.SaleListId});}
