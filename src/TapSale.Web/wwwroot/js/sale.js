(() => {
  ...
  const confirmInApp = (message, options = {}) => new Promise(resolve => {
    document.dispatchEvent(new CustomEvent('tapsale:confirm', { detail:{ message, options, resolve } }));
  });
  ...
  const deviceToken = localStorage.tapsaleDeviceToken ||= crypto.randomUUID();
  ...
  let restaurantPreference = localStorage.getItem('tapsaleRestaurantMode');
  const cartKey = () => `tapsale-cart-${userId}-${listId}`;
  const categoryKey = () => `tapsale-category-${userId}-${listId}`;
  ...
  function render() {
    const mode = localStorage.tapsaleView || 'tiles';
    grid.innerHTML = '';
  ...
  document.querySelectorAll('[data-view]').forEach(button => button.onclick = () => {
    localStorage.tapsaleView = button.dataset.view;
  ...
  restaurantToggle?.addEventListener('click', () => {
    ...
    localStorage.setItem('tapsaleRestaurantMode', restaurantPreference);
    applyRestaurantMode();
  });
  restaurantMedia.addEventListener('change', () => { if (restaurantPreference === null) applyRestaurantMode(); });
  document.querySelector(`[data-view="${localStorage.tapsaleView || 'tiles'}"]`)?.click();
  ...
  const openDb = () => new Promise((resolve, reject) => {
    // The legacy database name preserves pending offline sales created before the rename.
